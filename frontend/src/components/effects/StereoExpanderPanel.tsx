...
function CrossoverColumn({
  ...
  step,
  onChange,
}: CrossoverColumnProps) {
  const displayValue =
    unit === "kHz" ? (value / 1000).toFixed(1) : value.toString();
...
export function StereoExpanderPanel({
  config,
  onUpdate,
  onReset,
}: StereoExpanderPanelProps) {
  return (
