...
export function MobileVolumeControl({ volume, onVolumeChange, isMuted, onToggleMute, max = 2 }: MobileVolumeControlProps) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      const newVolume = Math.max(MIN_LINEAR, Math.min(max, dragStartVolume.current + deltaVolume));
  ...
  const iconPercentage = Math.max(0, logVolume - 0.5 * UNUSED_PORTION) / (1 - UNUSED_PORTION) * 100;
  ...
  return (
    <div
      ...
      title={isMuted ? 'Muted (click to unmute)' : `Volume: ${volumePercentage}% (click to mute)`}
    >
      {isMuted ? (
