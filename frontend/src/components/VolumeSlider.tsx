import { useCallback, useId, useRef, useState } from "react";

export type VolumeSliderSize = "small" | "large";

export interface VolumeSliderProps {
	volume: number; // 0 to 2 (linear)
	onVolumeChange: (volume: number) => void;
	size?: VolumeSliderSize;
	isMuted?: boolean;
	onDoubleClick?: () => void;
	showUnityMarker?: boolean; // Show a visual marker at volume=1.0 (0dB)
}

/**
 * Reusable volume slider with triangular visualization.
 *
 * Features:
 * - Wide, short triangle shape as range slider area
 * - Range: 0 to 2 (where 1.0 = 0dB unity gain)
 * - Exponential volume scaling for better perceptual control
 * - WCAG-compliant with keyboard support and ARIA labels
 * - Two sizes: "large" (40px) and "small" (28px)
 * - Optional mute state visualization
 * - Optional double-click handler for reset functionality
 * - Optional unity marker at volume=1.0
 */
export function VolumeSlider({
	volume,
	onVolumeChange,
	size = "large",
	isMuted = false,
	onDoubleClick,
	showUnityMarker = false,
}: VolumeSliderProps) {
	const [isDragging, setIsDragging] = useState(false);
	const svgRef = useRef<SVGSVGElement>(null);
	const uniqueId = useId(); // Generate unique ID for this slider instance

	// Triangle dimensions (wide and short)
	const width = 200;
	const height = size === "large" ? 40 : 28;
	const padding = 8; // Padding to prevent clipping of stroke and handle
	const cornerRadius = size === "large" ? 8 : 6;

	/**
	 * Convert linear volume (0-2) to exponential (perceived volume).
	 * Uses power curve: perceived = (linear/2)^2 * 2
	 * This makes the slider feel more natural to users.
	 */
	const linearToExponential = (linear: number): number => {
		// Normalize to 0-1, apply curve, scale back to 0-2
		const normalized = linear / 2;
		return (normalized ** 2) * 2;
	};

	/**
	 * Convert exponential (perceived) volume to linear (0-2).
	 * Inverse of linearToExponential
	 */
	const exponentialToLinear = (exponential: number): number => {
		// Normalize to 0-1, apply inverse curve, scale back to 0-2
		const normalized = exponential / 2;
		return Math.sqrt(Math.max(0, Math.min(1, normalized))) * 2;
	};

	// Current position on slider (0-1, where 0 is left/silent, 1 is right/max)
	const sliderPosition = exponentialToLinear(volume) / 2; // Normalize to 0-1 for positioning

	/**
	 * Handle mouse/touch position and convert to volume value.
	 * The triangle is wider at the base (right side = louder).
	 */
	const handlePointerMove = useCallback(
		(clientX: number) => {
			if (!svgRef.current) return;

			const rect = svgRef.current.getBoundingClientRect();
			const x = clientX - rect.left;

			// Clamp position to triangle bounds (0-1)
			const normalizedX = Math.max(0, Math.min(1, x / width));

			// Convert position to exponential volume (0-2)
			const newVolume = linearToExponential(normalizedX * 2);
			onVolumeChange(newVolume);
		},
		[onVolumeChange, width],
	);

	const handlePointerDown = useCallback(
		(e: React.PointerEvent) => {
			setIsDragging(true);
			handlePointerMove(e.clientX);
			// Capture pointer to continue receiving events outside element
			e.currentTarget.setPointerCapture(e.pointerId);
		},
		[handlePointerMove],
	);

	const handlePointerMoveEvent = useCallback(
		(e: React.PointerEvent) => {
			if (isDragging) {
				handlePointerMove(e.clientX);
			}
		},
		[isDragging, handlePointerMove],
	);

	const handlePointerUp = useCallback((e: React.PointerEvent) => {
		setIsDragging(false);
		e.currentTarget.releasePointerCapture(e.pointerId);
	}, []);

	// Keyboard support for accessibility
	const handleKeyDown = useCallback(
		(e: React.KeyboardEvent) => {
			let delta = 0;

			switch (e.key) {
				case "ArrowRight":
				case "ArrowUp":
					delta = 0.05; // 5% increase
					e.preventDefault();
					break;
				case "ArrowLeft":
				case "ArrowDown":
					delta = -0.05; // 5% decrease
					e.preventDefault();
					break;
				case "Home":
					onVolumeChange(0);
					e.preventDefault();
					return;
				case "End":
					onVolumeChange(2);
					e.preventDefault();
					return;
			}

			if (delta !== 0) {
				const newPosition = Math.max(0, Math.min(1, sliderPosition + delta));
				const newVolume = linearToExponential(newPosition * 2);
				onVolumeChange(newVolume);
			}
		},
		[sliderPosition, onVolumeChange],
	);

	const displayVolume = volume;
	const displayPosition = sliderPosition;

	// Map visual position to 10%-90% of width to keep indicator within triangle bounds
	const visualPosition = 0.1 + displayPosition * 0.85;

	// Unity marker position (volume=1.0 -> position=0.5 since range is 0-2)
	const unityPosition = 0.1 + 0.5 * 0.85;

	// Convert volume to dB for accessibility
	const volumeDb =
		volume === 0 ? -Infinity : 20 * Math.log10(volume);

	return (
		<svg
			ref={svgRef}
			width={width + padding * 2}
			height={height + padding * 2}
			viewBox={`${-padding} ${-padding} ${width + padding * 2} ${height + padding * 2}`}
			className="volume-slider-triangle"
			onPointerDown={handlePointerDown}
			onPointerMove={handlePointerMoveEvent}
			onPointerUp={handlePointerUp}
			onPointerCancel={handlePointerUp}
			onKeyDown={handleKeyDown}
			onDoubleClick={onDoubleClick}
			tabIndex={0}
			role="slider"
			aria-label="Volume"
			aria-valuemin={0}
			aria-valuemax={200}
			aria-valuenow={Math.round(displayVolume * 100)}
			aria-valuetext={volumeDb === -Infinity ? "-∞ dB" : `${volumeDb.toFixed(1)} dB`}
			style={{ cursor: isDragging ? "grabbing" : "pointer" }}
		>
			{/* Rounded triangle background (outline) */}
			<path
				d={`
              M ${cornerRadius},${height}
              L ${width - cornerRadius},${height}
              Q ${width},${height} ${width},${height - cornerRadius}
              L ${width},${cornerRadius}
              Q ${width},0 ${width - cornerRadius},0
              L ${cornerRadius},${height - cornerRadius}
              Q 0,${height} ${cornerRadius},${height}
              Z
            `}
				fill="rgba(0, 0, 0, 0.5)"
				stroke="rgba(255, 255, 255, 0.2)"
				strokeWidth="1"
			/>

			{/* Filled portion of triangle (shows current volume) */}
			<defs>
				<clipPath id={`volume-clip-${uniqueId}`}>
					<rect x="0" y="0" width={visualPosition * width} height={height} />
				</clipPath>
				<linearGradient
					id={`line-gradient-${uniqueId}`}
					x1="0"
					x2="0"
					y1="-8"
					y2={height}
					gradientUnits="userSpaceOnUse"
				>
					<stop
						offset="0%"
						stopColor={isMuted ? "#ff6666" : "#4a9eff"}
						stopOpacity="0"
					/>
					<stop
						offset="25%"
						stopColor={isMuted ? "#ff6666" : "#4a9eff"}
						stopOpacity="0.8"
					/>
					<stop
						offset="100%"
						stopColor={isMuted ? "#ff6666" : "#4a9eff"}
						stopOpacity="0.8"
					/>
				</linearGradient>
				<linearGradient
					id={`volume-fill-gradient-${uniqueId}`}
					x1="0"
					x2="1"
					y1="0"
					y2="0"
					gradientUnits="objectBoundingBox"
				>
					<stop offset="0%" stopColor="#1e90ff" stopOpacity="0.2" />
					<stop offset="60%" stopColor="#17ababff" stopOpacity="0.6" />
					<stop offset="80%" stopColor="#009714ff" stopOpacity="0.87" />
					<stop offset="98%" stopColor="#009714ff" stopOpacity="1" />
					<stop offset="100%" stopColor="#c01d17ff" stopOpacity="0.3" />
				</linearGradient>
				<linearGradient
					id={`volume-fill-gradient-muted-${uniqueId}`}
					x1="0"
					x2="1"
					y1="0"
					y2="0"
					gradientUnits="objectBoundingBox"
				>
					<stop offset="0%" stopColor="#ff4444" stopOpacity="0.2" />
					<stop offset="100%" stopColor="#ff4444" stopOpacity="0.4" />
				</linearGradient>
			</defs>
			<path
				d={`
              M ${cornerRadius},${height}
              L ${width - cornerRadius},${height}
              Q ${width},${height} ${width},${height - cornerRadius}
              L ${width},${cornerRadius}
              Q ${width},0 ${width - cornerRadius},0
              L ${cornerRadius},${height - cornerRadius}
              Q 0,${height} ${cornerRadius},${height}
              Z
            `}
				fill={
					isMuted
						? `url(#volume-fill-gradient-muted-${uniqueId})`
						: `url(#volume-fill-gradient-${uniqueId})`
				}
				clipPath={`url(#volume-clip-${uniqueId})`}
			/>

			{/* Unity marker (0dB) - optional */}
			{showUnityMarker && (
				<line
					x1={unityPosition * width}
					y1={0}
					x2={unityPosition * width}
					y2={height}
					stroke="rgba(255, 255, 255, 0.3)"
					strokeWidth="1"
					strokeDasharray="2,2"
				/>
			)}

			{/* Volume indicator line - extends above and fades out */}
			<line
				x1={visualPosition * width}
				y1={-padding}
				x2={visualPosition * width}
				y2={height}
				stroke={`url(#line-gradient-${uniqueId})`}
				strokeWidth="2"
			/>

			{/* Draggable handle */}
			<circle
				cx={visualPosition * width}
				cy={height - displayPosition * height}
				r={size === "large" ? 6 : 4}
				fill={isMuted ? "#ff6666" : "#4a9eff"}
				stroke="#fff"
				strokeWidth="1.5"
				style={{ cursor: isDragging ? "grabbing" : "grab" }}
			/>
		</svg>
	);
}
