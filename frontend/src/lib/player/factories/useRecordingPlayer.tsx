...
import { useStemPlayer, type UseStemPlayerResult } from "../../../hooks/useStemPlayer";
	...
	const Ruler: React.FC<RulerComponentProps> = useMemo(
		() =>
			({ height, className }) => {
				...
				if (!player) throw new Error("Ruler must be used within RecordingPlayer context");
	...
	const Waveforms: React.FC<WaveformsComponentProps> = useMemo(
		() =>
			({ mode = "split", className, renderStemControls }) => {
				const player = useContext(RecordingPlayerContext);
				if (!player) throw new Error("Waveforms must be used within RecordingPlayer context");
