...
export function useProfileFiles(profileName: string | undefined) {
    ...
    return useQuery({
        ...
        select: (data) => data.map(file => ({...file, displayName: file.display_name || file.name}))
    })
}
...
export function useRecording(recordingId: string | undefined) {
    ...
    return useQuery({
        ...
        gcTime: 5 * 60 * 1000,
        select: (data) => data ? ({...data, displayName: data.display_name || data.name}) : null,
    })
}

export function useUpdateDisplayName() {
    const queryClient = useQueryClient()
    const mutationOptions = apiProfilesProfileNameFilesOutputNameDisplayNameUpdateDisplayNameMutation();

    return useMutation({
        ...mutationOptions,
        onSuccess: (_data, variables) => {
            queryClient.invalidateQueries({
                queryKey: apiProfilesProfileNameFilesGetProfileFilesQueryKey({path: {profile_name: variables.path.profile_name}})
            })
        },
    })
}