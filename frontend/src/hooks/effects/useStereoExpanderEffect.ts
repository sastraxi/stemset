...
export function useStereoExpanderEffect({
  ...
  useEffect(() => {
    ...
    async function loadWorklet() {
      try {
        ...
        console.error(
          "[useStereoExpanderEffect] Failed to load AudioWorklet:",
          error,
        );
        console.error(
          "[useStereoExpanderEffect] AudioContext state:",
          audioContext?.state,
        );
        console.error(
          "[useStereoExpanderEffect] AudioContext sample rate:",
          audioContext?.sampleRate,
        );
        console.error(
          "[useStereoExpanderEffect] Worklet loaded ref:",
  ...
  useEffect(() => {
    ...
    workletNodeRef.current.parameters
      .get("lowMidCrossover")
      ?.setTargetAtTime(lowMidCrossover, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("midHighCrossover")
      ?.setTargetAtTime(midHighCrossover, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("expLow")
      ?.setTargetAtTime(expLow, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("compLow")
      ?.setTargetAtTime(compLow, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("expMid")
      ?.setTargetAtTime(expMid, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("compMid")
      ?.setTargetAtTime(compMid, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("expHigh")
      ?.setTargetAtTime(expHigh, audioContext.currentTime, 0.01);
    workletNodeRef.current.parameters
      .get("compHigh")
      ?.setTargetAtTime(compHigh, audioContext.currentTime, 0.01);
