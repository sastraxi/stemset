...
export function useReverbEffect({
  ...
  useEffect(() => {
    ...
    async function loadImpulseResponse() {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch impulse response: ${response.statusText}`,
          );
        }
        ...
        console.error(
          "[useReverbEffect] Failed to load impulse response:",
          error,
        );
        console.error(
          "[useReverbEffect] Failed to load impulse response details:",
          {
            responseOk: false,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(
        "[useReverbEffect] Reverb audio graph connected successfully",
      );
      ...
      if (impulseBufferRef.current && convolver) {
        try {
          ...
          console.error(
            "[useReverbEffect] Failed to set convolver buffer:",
            error,
          );
        }
      }
  ...
  useEffect(() => {
    if (!audioContext || !isReady || !wetGainRef.current || !dryGainRef.current)
      return;
