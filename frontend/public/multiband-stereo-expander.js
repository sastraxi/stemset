class MultiBandStereoExpander extends AudioWorkletProcessor {
    ...
    clamp(x, min, max) { return Math.max(min, Math.min(x, max)); }
    ...
    compress(x, envState, amt) {
        ...
        let coef = absx > envState ? Math.exp(-1 / (attack * sampleRate)) : Math.exp(-1 / (release * sampleRate));
    ...
    process(inputs, outputs, parameters) {
        ...
        const exp = [parameters.expLow[0], parameters.expMid[0], parameters.expHigh[0]];
        const comp = [parameters.compLow[0], parameters.compMid[0], parameters.compHigh[0]];
        const rot = (parameters.rot ? parameters.rot[0] : 0) * 0.017453292;
        const reverbBlend = parameters.reverbBlend ? parameters.reverbBlend[0] : 0.3;
        ...
        for (let b = 0; b < 3; ++b) {
            for (let i = 0; i < n; ++i) {
                ...
                if (width > 1 && this.reverbBuffer) {
                    ...
                    let dryL = M + S, dryR = M - S;
                    let idx = (this.reverbPtr + i - 1 + this.inLhist.length) % this.inLhist.length;
